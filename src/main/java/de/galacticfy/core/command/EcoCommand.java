...
public class EcoCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
        ...
        // Account sicherstellen (Name updaten)
    ...
    private void handleSet(CommandSource src, Player target, long amount) {
        boolean ok = economy.setBalance(target.getUniqueId(), amount);
        if (!ok) {
            src.sendMessage(prefix().append(Component.text("§cFehler beim Setzen der Balance.")));
            return;
        }
    ...
    private void handleAdd(CommandSource src, Player target, long amount) {
        ...
        boolean ok = economy.deposit(target.getUniqueId(), amount);
        if (!ok) {
            src.sendMessage(prefix().append(Component.text("§cFehler beim Hinzufügen von Guthaben.")));
            return;
        }

        long newBal = economy.getBalance(target.getUniqueId());
    ...
    private void handleRemove(CommandSource src, Player target, long amount) {
        ...
        long current = economy.getBalance(target.getUniqueId());
        if (current < amount) {
            src.sendMessage(prefix().append(Component.text(
        ...
        boolean ok = economy.withdraw(target.getUniqueId(), amount);
        if (!ok) {
            src.sendMessage(prefix().append(Component.text(
                    "§cFehler beim Entfernen von Guthaben."
            )));
            return;
        }
    ...
    public List<String> suggest(Invocation invocation) {
        ...
        // Betrag → kein Vorschlag
        return List.of();
    }

    @Override
    public boolean hasPermission(Invocation invocation) {
        // Sichtbar für alle, aber echte Rechte checken wir in isAdmin(...)
        return true;
    }
}
